...
async function screenshots(url: string, name: string) {
  ...
  await page.setViewport({
    height: Math.round(347 * 4),
    width: Math.round(260 * 4),
    deviceScaleFactor: 2,
  });
  await page.goto(url, { waitUntil: "networkidle2" });
  ...
  await page.setViewport({
    height: Math.round(301 * 4),
    width: Math.round(482 * 4),
    deviceScaleFactor: 2,
  });
  await page.goto(url, { waitUntil: "networkidle2" });
  ...
  return [name + "-mobile.png", name + "-ipad.png", name + "-laptop.png"];
}
